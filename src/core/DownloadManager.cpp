...
//      LOG_DEBUG ("%s: interface was specified as ANY, picked %s",__func__,DownloadManager::connectionId2Name(interface).c_str());
    ...
    if (createTempFile == false) { // only if filename is provided
        ...
        if (overrideTargetFile.empty()) {
            ...
            if (g_file_test(finalFilePath.c_str(), G_FILE_TEST_EXISTS) || g_file_test(tmpFilePath.c_str(), G_FILE_TEST_EXISTS)) {
...
void DownloadManager::startupGlibCurl()
{
    ...
    if (CURLM_OK != retVal) {
        LOG_WARNING_PAIRS(LOGID_CURL_FAIL_MAXCONNECTION, 1, PMLOGKFV("error code", "%d", retVal), "curl_multi_setopt: CURLMOPT_MAXCONNECTS failed");
    }
//
...
uint32_t DownloadManager::uploadPOSTFile(const std::string& file, const std::string& url, const std::string& filePostLabel, std::vector<PostItem>& postHeaders, std::vector<std::string>& httpHeaders, std::vector<kvpair>& cookies, const std::string& contentType)
{
...
uint32_t DownloadManager::uploadPOSTBuffer(const std::string& buffer, const std::string& url, std::vector<std::string>& httpHeaders, const std::string& contentType)
{
...
void DownloadManager::postUploadStatus(uint32_t id, const std::string& sourceFile, const std::string& url, bool completed, CURLcode curlCode, uint32_t httpCode, const std::string& responseString,
